...
class PokeRed:
    STATS = { 
        ...
        "Defense EV":           { "address": 0xD180, "length": 2,   "type": "int",      "is_poke_stat": True,   "amount": 1 },
        "Speed EV":             { "address": 0xD182, "length": 2,   "type": "int",      "is_poke_stat": True,   "amount": 1 },
        ...
        "Defense":              { "address": 0xD191, "length": 2,   "type": "int",      "is_poke_stat": True,   "amount": 1 },
        "Speed":                { "address": 0xD193, "length": 2,   "type": "int",      "is_poke_stat": True,   "amount": 1 },
        "Special":              { "address": 0xD195, "length": 2,   "type": "int",      "is_poke_stat": True,   "amount": 1 },
        "Nickname":             { "address": 0xD2B5, "length": 10,  "type": "string",   "is_poke_stat": True,   "amount": 1 },
    }
    ...
    def __init__(self, gb_path, state_file = None, head="headless", hide_window=False, tick_callback=None):
        self.pyboy = PyBoy(
            gb_path,
            debugging=False,
            disable_input=False,
    ...
    def get_stat(self, info_name, pokemon_index=0, info_index=0, opponent=False):
        ...
        if not self.STATS[info_name]["is_poke_stat"] and (pokemon_index > 0 or opponent):
            print(f"Error: {info_name} is not a valid pokemon info")
            return None
        
        if info_index >= self.STATS[info_name]["amount"]:
            print(f"Error: {info_name} does not have {info_index + 1} amount of {info_name} (index {info_index})")
            return None
    ...
    def get_poke_info(self, info, info_index=0, opponent=False):
        if not info in self.STATS:
            ...
            return None
        return [self.get_stat(info, pokemon_index=i, info_index=info_index, opponent=opponent) for i in range(6)]
