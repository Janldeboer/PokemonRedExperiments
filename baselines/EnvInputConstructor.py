...
class EnvInputConstructor:
    ...
    ENV_CONFIG = {
        ...
        "observation_space": spaces.Box(low=0, high=255, shape=(46, 40, 1), dtype=np.uint8),
    }
    
    def __init__(self):
        apply_dict_as_attributes(self, self.ENV_CONFIG)
        self.pad = np.zeros(shape=(self.mem_padding, self.combined_shape[1], 1), dtype=np.uint8)
    ...
    def render_for_ml(self, stats, frame, last_rewards):
        ...
        full_ml_image =  np.concatenate((rendered_infobars, self.pad, rendered_frame), axis=0)
    ...
    def get_infobars(self, last_rewards):
        info_bars = self.create_info_bars(last_rewards, self.combined_shape[1], self.mem_height, self.col_steps)
        return info_bars
